...
def gerar_msg_criados_hoje(df: pd.DataFrame, mapa: dict, data_ref: str, hora_ref: str, hoje) -> str | None:
    ...
    # 2. Filtra por data de criação = hoje (todos os tipos de serviço)
    print(f"[DEBUG-CRIACAO] hoje={hoje} tipo={type(hoje)}, total_linhas={len(df2)}")
    if col_criacao:
        df2["_dt_criacao"] = normalizar_data(df2[col_criacao])
        amostra = df2["_dt_criacao"].dropna().head(5).tolist()
        print(f"[DEBUG-CRIACAO] Amostra datas criacao: {amostra}")
        matches = (df2["_dt_criacao"] == hoje).sum()
        print(f"[DEBUG-CRIACAO] Linhas com data=hoje: {matches}")
        df2 = df2[df2["_dt_criacao"] == hoje]
    else:
        print("[MSG-CRIACAO] ⚠️  Coluna 'Data de Criação' não encontrada.")

    if df2.empty:
        print("[MSG-CRIACAO] Nenhum agendamento criado hoje.")
        return None
