...
def baixar_excel(page) -> str:
    ...
    try:
        ...
        # Sempre usa nome com timestamp para evitar sobrescrever arquivo anterior
        ts = datetime.now().strftime('%Y%m%d_%H%M%S')
        nome_arquivo = f"Export_{ts}.xlsx"
...
def main():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # ── 7. Clica BUSCA para forçar refresh dos dados ──────────
            buscar_e_aguardar(page)

            # ── 8. Reabre filtro e faz download ──────────────────────
            expandir_filtro(page)
    ...
    # ── Imagem 1: Criados Hoje  (filtra DATA CRIAÇÃO = hoje) ─────
    ...
    # ── Imagem 2: Próximos Dias (filtra DATA AGENDADA >= hoje) ───
    ...
    msg2 = gerar_msg_proximos_dias(df, mapa, data_ref, hora_ref, hoje)
    if caminho_img:
