...
def baixar_excel(page) -> str:
    ...
    try:
        ...
        with page.expect_download(timeout=60000) as download_info:
            ...
            # Modal de confirmação: aguarda até 20s pelo botão CONFIRMAR
            confirmado = False
            for tentativa in range(20):
                ...
                try:
                    ...
                    if btn.is_visible(timeout=500):
                        btn.click()
                        print(f"  CONFIRMAR clicado (tentativa {tentativa+1}).")
                        confirmado = True
                        break
                except Exception:
                    pass
            if not confirmado:
                print("  CONFIRMAR não apareceu em 20s. Botões visíveis na página:")
                try:
                    btns = page.locator("button").all()
                    textos = [b.text_content() or "" for b in btns[:15]]
                    print(f"  {textos}")
                except Exception as eb:
                    print(f"  (erro ao listar botões: {eb})")
