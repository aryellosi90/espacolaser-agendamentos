...
Fluxo:
  ...
  3. Download 1 — DATA PROGRAMADA = hoje → hoje+14 (para Próximos Dias)
  4. Download 2 — DATA CRIAÇÃO = hoje (para Criados Hoje)
  5. Gera imagem PNG de cada relatório
  6. Envia via Webhook N8N → Z-API → WhatsApp
"""
...
def limpar_data_criacao(page):
    """Limpa o campo Data de Criação (volta ao estado sem filtro)."""
    for sel in ["input[name='txtCreateDate']", "input[id*='CreateDate']"]:
        try:
            ...
            if loc.is_visible(timeout=1500):
                ...
                loc.fill("")
                page.wait_for_timeout(300)
                print(f"  [DATA-CRIACAO] Campo limpo via {sel!r}")
                return
        except Exception:
            continue
    page.evaluate("""
...
def gerar_msg_criados_hoje(df: pd.DataFrame, mapa: dict, data_ref: str, hora_ref: str, hoje) -> str | None:
    ...
    # 2. Filtra Localidade = AVALIAÇÃO
    if col_localidade:
        df2 = df2[df2[col_localidade].str.upper().str.contains("AVALIA", na=False)]

    # 3. Filtra por Data de Criação = hoje
    if col_criacao:
        datas = pd.to_datetime(df2[col_criacao], dayfirst=True, errors="coerce")
        hoje_ts = pd.Timestamp(hoje)
        df2 = df2[datas.dt.normalize() == hoje_ts]
    else:
...
def gerar_imagem_criados_hoje(df: pd.DataFrame, mapa: dict, hoje, data_ref: str, hora_ref: str):
    ...
    df2 = df.copy()
    print(f"[DEBUG-IMG] Total antes filtros: {len(df2)}")
    if col_status:
        df2 = df2[~df2[col_status].str.upper().str.contains("CANCEL", na=False)]
        print(f"[DEBUG-IMG] Após remover cancelados: {len(df2)}")
    if col_localidade:
        vals = df2[col_localidade].dropna().unique().tolist()
    ...
    if col_criacao:
        ...
        datas = pd.to_datetime(df2[col_criacao], dayfirst=True, errors="coerce")
        hoje_ts = pd.Timestamp(hoje)
        print(f"[DEBUG-IMG] hoje_ts={hoje_ts}, amostra normalizada: {datas.dt.normalize().dropna().head(3).tolist()}")
        df2 = df2[datas.dt.normalize() == hoje_ts]
    ...
    for estab_orig, grp_estab in df2.groupby(col_estab, sort=True):
        ...
        linhas.append((f"⊟ {nome}", total_loja, "estab"))
    ...
    COR = {
        ...
        "usuario_impar": ("#F5F5F5", "black"),
        "total":     ("#D9D9D9", "black"),
    }

    def celula(x, y, w, h, texto, bg, fg, bold=False, alinha="left"):
        ax.add_patch(plt.Rectangle(
            (x, y), w, h,
            facecolor=bg, edgecolor="#BBBBBB", linewidth=0.5, zorder=1
        ))
    ...
    # ── Cabeçalho ────────────────────────────────────────────────────
    y_cab = fig_h - pad - header_h
    celula(pad, y_cab, label_w, header_h, "Rótulos de Linha",
           COR["cabecalho"][0], COR["cabecalho"][1], True, "left")
    celula(pad + label_w, y_cab, val_w, header_h, "Contagem de Cliente",
           COR["cabecalho"][0], COR["cabecalho"][1], True, "center")

    # ── Linhas de dados ───────────────────────────────────────────────
    usr_idx = 0
    for ri, (label, val, estilo) in enumerate(linhas):
        y_row = fig_h - pad - header_h - (ri + 1) * row_h

        if estilo == "estab":
            bg, fg, bold = COR["estab"][0], COR["estab"][1], True
            usr_idx = 0
        elif estilo == "total":
            bg, fg, bold = COR["total"][0], COR["total"][1], True
        else:
            chave = "usuario_par" if usr_idx % 2 == 0 else "usuario_impar"
            bg, fg, bold = COR[chave][0], COR[chave][1], False
            usr_idx += 1

        indent = "    " if estilo == "usuario" else ""
        celula(pad, y_row, label_w, row_h, f"{indent}{label}", bg, fg, bold, "left")
        celula(pad + label_w, y_row, val_w, row_h, str(val), bg, fg, bold, "center")
    ...
    print(f"[IMG-CRIACAO] Salvando em: {caminho}")
    plt.savefig(caminho, dpi=150, bbox_inches="tight", facecolor="white")
    plt.close()

    if not os.path.exists(caminho):
        print(f"[IMG-CRIACAO] ERRO: arquivo não criado em {caminho}")
        return None, None

    with open(caminho, "rb") as f:
        img_b64 = "data:image/png;base64," + base64.b64encode(f.read()).decode()

    print(f"[IMG-CRIACAO] Imagem gerada com sucesso: {caminho}")
    return caminho, img_b64
...
def main():
    ...
    hoje            = agora_sp.date()
    data_fim_excel  = hoje + timedelta(days=DIAS_FRENTE_EXCEL)
    inicio_str      = hoje.strftime("%d/%m/%Y")
    fim_str         = data_fim_excel.strftime("%d/%m/%Y")

    print(f"\n{'='*55}")
    print(f"  Bot Agendamentos — {data_ref} {hora_ref}")
    print(f"  Download 1 (Próximos Dias): DATA PROGRAMADA {inicio_str} → {fim_str}")
    print(f"  Download 2 (Criados Hoje) : DATA CRIAÇÃO = {inicio_str}")
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # ════════════════════════════════════════════════════════
            # DOWNLOAD 1 — DATA PROGRAMADA = hoje até +14
            #              (para relatório Próximos Dias)
            # ════════════════════════════════════════════════════════
            print("\n[DOWNLOAD-1] Próximos Dias (DATA PROGRAMADA)")
            ...
            # Re-abre filtro após × e faz download
            expandir_filtro(page)
            caminho_proximos = baixar_excel(page)
            print(f"[DOWNLOAD-1] Salvo em: {caminho_proximos}")

            # ════════════════════════════════════════════════════════
            # DOWNLOAD 2 — DATA CRIAÇÃO = hoje
            #              (para relatório Criados Hoje)
            # ════════════════════════════════════════════════════════
            print("\n[DOWNLOAD-2] Criados Hoje (DATA CRIAÇÃO)")

            # Navega novamente para limpar estado do relatório
            navegar_para_agendamentos(page)
            expandir_filtro(page)

            # Limpa DATA PROGRAMADA (deixa campo em branco ou amplo)
            # e define DATA CRIAÇÃO = hoje
            configurar_data_criacao(page, inicio_str)

            # Mantém mesmos campos de detalhe
            configurar_nivel_detalhamento(page)
            limpar_coluna_anterior(page)

            expandir_filtro(page)
            caminho_criados = baixar_excel(page)
            print(f"[DOWNLOAD-2] Salvo em: {caminho_criados}")
    ...
    # ── Parsear Excel 1: Próximos Dias ──────────────────────────
    print("\n[PARSE-1] Próximos Dias")
    df_proximos = None
    mapa_proximos = None
    if caminho_proximos:
        try:
            df_proximos   = ler_excel(caminho_proximos)
            mapa_proximos = mapear_colunas(df_proximos)
        except Exception as e:
            print(f"  ERRO ao ler Excel 1: {e}")

    # ── Parsear Excel 2: Criados Hoje ───────────────────────────
    print("\n[PARSE-2] Criados Hoje")
    df_criados = None
    mapa_criados = None
    if caminho_criados:
        try:
            df_criados   = ler_excel(caminho_criados)
            mapa_criados = mapear_colunas(df_criados)
            # Debug rápido para confirmar dados
            col_cri = mapa_criados.get("data_criacao")
            if col_cri:
                datas_c = pd.to_datetime(df_criados[col_cri], dayfirst=True, errors="coerce")
                n_hoje  = (datas_c.dt.date == hoje).sum()
                print(f"  Total linhas: {len(df_criados)} | DATA CRIAÇÃO=hoje: {n_hoje}")
        except Exception as e:
            print(f"  ERRO ao ler Excel 2: {e}")

    # ── Imagem 1: Criados Hoje ───────────────────────────────────
    print("\n--- GERANDO: Criados Hoje ---")
    if df_criados is not None and mapa_criados is not None:
        caminho1, img_b64_criados = gerar_imagem_criados_hoje(
            df_criados, mapa_criados, hoje, data_ref, hora_ref
        )
        if img_b64_criados:
            print(f"  Imagem: {caminho1}")
            enviar_webhook("Agendamentos Criados Hoje", "criados_hoje", data_ref, hora_ref, img_b64_criados)
        else:
            print("  Sem dados para 'Criados Hoje'.")
    else:
        print("  Excel 2 não disponível.")

    # ── Imagem 2: Próximos Dias ──────────────────────────────────
    print("\n--- GERANDO: Próximos Dias ---")
    if df_proximos is not None and mapa_proximos is not None:
        try:
            caminho_img, img_b64 = gerar_imagem_proximos_dias(
                df_proximos, mapa_proximos, hoje, data_ref, hora_ref
            )
        except Exception as e:
            print(f"  ERRO ao gerar imagem: {e}")
            caminho_img, img_b64 = None, None

        msg2 = gerar_msg_proximos_dias(df_proximos, mapa_proximos, data_ref, hora_ref, hoje)
        if caminho_img:
