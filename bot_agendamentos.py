...
def main():
    ...
    hoje            = agora_sp.date()
    data_fim_exibir = hoje + timedelta(days=DIAS_FRENTE)
    data_fim_excel  = hoje + timedelta(days=DIAS_FRENTE_EXCEL)
    inicio_str      = hoje.strftime("%d/%m/%Y")
    fim_str         = data_fim_excel.strftime("%d/%m/%Y")   # range amplo para Excel

    print(f"\n{'='*55}")
    print(f"  Bot Agendamentos — {data_ref} {hora_ref}")
    print(f"  Período Excel: {inicio_str} → {fim_str} ({DIAS_FRENTE_EXCEL}d)")
    print(f"  Próximos Dias: {inicio_str} → {data_fim_exibir.strftime('%d/%m/%Y')} ({DIAS_FRENTE}d)")
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # ── DEBUG: captura todos os inputs de data no filtro ─────
            try:
                inputs_data = page.locator("input[name*='Date'], input[name*='date'], input[class*='daterange']").all()
                for inp in inputs_data:
                    nome = inp.get_attribute("name") or "?"
                    val  = inp.input_value()
                    print(f"[DEBUG-FILTER] Campo data: name={nome}, valor='{val}'")
            except Exception as e:
                print(f"[DEBUG-FILTER] Erro ao ler inputs: {e}")
    ...
    # ── 6b. DEBUG completo do Excel ──────────────────────────────
    print("\n--- DEBUG EXCEL ---")
    print(f"  agora_sp  = {agora_sp}")
    print(f"  hoje      = {hoje}  (type={type(hoje).__name__})")
    col_criacao_dbg = mapa.get("data_criacao")
    col_agendada_dbg = mapa.get("data_agendada")
    col_local_dbg   = mapa.get("localidade")
    print(f"  col_data_criacao  = {col_criacao_dbg}")
    print(f"  col_data_agendada = {col_agendada_dbg}")
    print(f"  col_localidade    = {col_local_dbg}")
    if col_criacao_dbg:
        datas_dbg = pd.to_datetime(df[col_criacao_dbg], dayfirst=True, errors="coerce")
        count_hoje_dbg = (datas_dbg.dt.date == hoje).sum()
        print(f"  Total linhas Excel          : {len(df)}")
        print(f"  Linhas c/ DATA CRIAÇÃO=hoje : {count_hoje_dbg}")
        print(f"  DATA CRIAÇÃO mín            : {datas_dbg.min()}")
        print(f"  DATA CRIAÇÃO máx            : {datas_dbg.max()}")
        contagem_data = datas_dbg.dt.date.value_counts().sort_index()
        print(f"  Contagem por DATA CRIAÇÃO (últimas 5): {contagem_data.tail(5).to_dict()}")
    if col_local_dbg:
        vals_loc = df[col_local_dbg].dropna().unique().tolist()
        print(f"  Valores Localidade          : {vals_loc}")
    print("--- FIM DEBUG EXCEL ---\n")

    # ── 7a. Imagem: Criados Hoje ──────────────────────────────
    print("\n--- GERANDO: Criados Hoje ---")
    caminho1, img_b64_criados = gerar_imagem_criados_hoje(df, mapa, hoje, data_ref, hora_ref)
    if img_b64_criados:
